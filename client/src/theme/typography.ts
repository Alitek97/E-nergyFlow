...
export const numberSizes: Record<NumberSizeTier, number> = {
  ...
  input: scaleValue(sizeTokens.lg),
  summary: scaleValue(sizeTokens.sm),
  output: scaleValue(sizeTokens.xl),
  final: scaleValue(34),
  kpi: scaleValue(34),
};

export const typeScale: Record<TypographyScaleToken, number> = {
  display: scaleValue(34),
  title: scaleValue(28),
  heading: scaleValue(20),
  subheading: scaleValue(17),
  body: scaleValue(15),
...
function getUIFamilyForLanguage(
  language: ResolvableLanguage,
...
function variant(
  scaleToken: TypographyScaleToken,
  weight: TextWeight,
  numeric = false,
...
export function createTypography(
  language: ResolvableLanguage = "en",
): TypographyFamilies {
  const getTextFamily = (
    weight: TextWeight = "regular",
    lang: ResolvableLanguage = language,
  ...
  const getVariantStyle = (
    ...
    return {
      ...def,
      fontFamily: def.numeric
        ? getNumberFamily(weight)
        : getTextFamily(weight, lang),
    };
  };
...
export function getFontFamilyForText(
  ...
  return isArabicText(text)
    ? families.getTextFamily(weight, "ar")
    : families.getTextFamily(weight, "en");
}
