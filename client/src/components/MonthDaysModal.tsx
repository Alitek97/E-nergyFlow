...
export function MonthDaysModal({
  ...
  const handleDelete = (day: DaySummary) => {
    ...
    const confirmMessage = t("delete_day_confirm").replace(
      "{{date}}",
      day.dateKey,
    );

    Alert.alert(t("delete_day"), confirmMessage, [
  ...
  const formatMonthParts = (monthKey: string) => {
    ...
    return {
      monthName: date.toLocaleDateString(isRTL ? "ar" : "en", {
        month: "long",
      }),
      year,
    };
    ...
    >
      <View
        style={[
          ...
          { backgroundColor: withAlpha(theme.backgroundRoot, 0.5) },
        ]}
      >
        <View
          style={[
            ...
            >
              {days.map((day) => {
                ...
                const productionText = formatEnergy(
                  day.production,
                  unitsConfig,
                  { prefer: "fixed", display: "smart" },
                );
                const flowText = formatEnergy(
                  Math.abs(day.exportVal),
                  unitsConfig,
                  { prefer: "fixed", display: "smart" },
                );
                return (
                  ...
                  >
                      ...
                      <View style={styles.dayStats}>
                        <View style={styles.dayStat}>
                          ...
                          <View style={styles.dayStatValueRow}>
                            <NumberText size="small">
                              {productionText.valueText}
                            </NumberText>
                            <ThemedText semanticVariant="unit">
                              {productionText.unitText}
                            </ThemedText>
                          </View>
                        </View>
                        {(() => {
                          const isExport = day.exportVal >= 0;
                          const flowStyle = getFlowLabelAndStyle(
                            isExport,
                            t,
                            theme,
                          );
                    ...
                    <Pressable
                      style={[
                        styles.deleteButton,
                        { backgroundColor: withAlpha(deleteColor, 0.12) },
