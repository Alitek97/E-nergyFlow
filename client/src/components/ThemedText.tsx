...
export function ThemedText({
  ...
  const themeTextColor =
    (theme as { colors?: { text?: string } }).colors?.text ?? theme.text;
  ...
  const buildSegmentedChildren = (value: React.ReactNode): React.ReactNode => {
    if (typeof value === "number") {
      ...
      return (
        <Text
          key={`seg-num-${text}`}
          style={{ fontFamily: typography.getNumberFamily(resolvedWeight) }}
        >
          {text}
        </Text>
      );
    ...
    if (typeof value === "string") {
      ...
      return parts.map((part, index) => {
        ...
        const familyName = numericPart
          ...
          : typography.getTextFamily(
              resolvedWeight,
              isArabicText(part) ? "ar" : "en",
            );
        return (
    ...
    if (Array.isArray(value)) {
      return value.map((child, index) => (
        <React.Fragment key={`seg-arr-${index}`}>
          {buildSegmentedChildren(child)}
        </React.Fragment>
      ));
    }
