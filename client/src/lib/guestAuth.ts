  ...
  try {
    ...
    if (anonError) {
      return {
        success: false,
        error:
          "Anonymous sign-in not enabled. Please enable it in Supabase Dashboard > Authentication > Providers > Anonymous Sign-In",
        requiresSetup: true,
      };
    }
  } catch {
    return {
      success: false,
      error:
        "Anonymous sign-in failed. Please enable it in Supabase Dashboard.",
      requiresSetup: true,
    };
  }
