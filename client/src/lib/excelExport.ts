...
export async function exportExcel(
  ...
  try {
    ...
    if (!cacheDir) {
      ...
      Alert.alert(
        "Export Failed",
        "Could not access app storage. Please try again.",
      );
      return false;
    }
    ...
    Alert.alert(
      "Export Failed",
      "Could not create the Excel file. Please try again.",
    );
    return false;
  }
}
...
export async function generateExcelReport(
  ...
  try {
    ...
    for (const f of FEEDERS) {
      const computed = feederRowComputed(currentDay, f);
      feedersData.push([f, computed.end ?? "", computed.diff ?? ""]);
    }
    ...
    for (const day of sortedDays) {
      ...
      const flowValue = stats.isExport
        ? stats.exportVal
        : -Math.abs(stats.exportVal);
      dailyData.push([
    ...
    const hasMixedModes = monthlyList.some(
      (m) => m.hasExport && m.hasWithdrawal,
    );
    ...
    if (hasMixedModes) {
      ...
      const allExport = monthlyList.every(
        (m) => m.hasExport && !m.hasWithdrawal,
      );
    ...
    setColumnWidths(
      monthlySheet,
      hasMixedModes ? [15, 18, 15, 18, 18, 20, 12] : [15, 18, 18, 18, 20, 12],
    );
    XLSX.utils.book_append_sheet(
      workbook,
      monthlySheet,
      t("monthly_summary_sheet"),
    );
...
export async function generateTextReport(
  ...
  try {
    ...
    for (const f of FEEDERS) {
      const computed = feederRowComputed(currentDay, f);
      const statusSuffix = computed.isStopped
        ? " [No Reading]"
        : computed.hasError
          ? " [Error]"
          : "";
      lines.push(
        `  ${f}: ${t("start")}: ${computed.start ?? "-"} → ${t("end")}: ${computed.end ?? "-"} (${t("difference")}: ${computed.diff === null ? "-" : round2(computed.diff)})${statusSuffix}`,
      );
    }
    lines.push("");
    ...
    for (const tb of TURBINES) {
      const computed = turbineRowComputed(currentDay, tb);
      lines.push(
        `  ${t("turbine")} ${tb}: ${t("previous")}: ${computed.prev ?? "-"} → ${t("present")}: ${computed.pres ?? "-"}`,
      );
      lines.push(
        `       ${t("hours")}: ${computed.hours}h | ${t("difference")}: ${computed.diff === null ? "-" : round2(computed.diff)} MWh | ${computed.mwPerHr === null ? "-" : round2(computed.mwPerHr)} MW/h`,
      );
    }
    lines.push("");
