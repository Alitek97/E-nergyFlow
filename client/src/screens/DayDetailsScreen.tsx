...
export default function DayDetailsScreen() {
  ...
  const hasMeaningfulFeeders = useMemo(
    () =>
      feederRows.some(
        (row) => row.start !== 0 || row.end !== 0 || row.diff !== 0,
      ),
    [feederRows],
  );
  ...
  const statusText =
    hasMeaningfulFeeders && hasMeaningfulTurbines ? "Complete" : "Partial";
  const statusColor =
    hasMeaningfulFeeders && hasMeaningfulTurbines
      ? theme.success
      : theme.warning;

  const formatValue = useCallback(
    (value: number): string => formatSmartNumber(value, unitsConfig),
    [unitsConfig],
  );
  const formatInt = useCallback(
    (value: number): string =>
      formatNumber(value, { ...unitsConfig, decimals: 0 }),
    [unitsConfig],
  );
  const sectionTitleAlignStyle = isRTL
    ? styles.sectionTitleRTL
    : styles.sectionTitleLTR;
    ...
    () => [
      {
        ...
        align: "center",
        renderCell: (value) => (
          <FeederCode code={String(value)} style={styles.turbineCodeCellText} />
        ),
      },
      {
        key: "previous",
        ...
        formatValue: (value) => formatValue(Number(value)),
        renderCell: (value) => (
          <MarqueeCell value={formatValue(Number(value))} active={isFocused} />
        ),
      },
      {
        key: "present",
      ...
      },
      {
        ...
        isNumeric: true,
        formatValue: (value) =>
          value !== null ? formatInt(Number(value)) : "-",
      },
      {
        key: "mwPerHr",
        ...
        formatValue: (value) =>
          value !== null
            ? formatPower(Number(value), unitsConfig, { display: "smart" })
                .valueText
            : "-",
      },
    ],
    [formatInt, formatValue, isFocused, t, unitsConfig],
  );
              ...
              >
                ...
                <View style={styles.energyStatsRow}>
                  {(() => {
                    ...
                    const consumptionText = formatEnergy(
                      totals.consumption,
                      unitsConfig,
                      {
                        prefer: "fixed",
                    ...
                    const flowValueText = flowStats
                      ? formatEnergy(Math.abs(flowStats.value), unitsConfig, {
                          prefer: "fixed",
                          precision: "adaptive",
                        })
                      : null;
                        ...
                        {flowStats ? (
                          <View style={styles.energyStatItem}>
                            <View
                              style={[
                                styles.statDot,
                                { backgroundColor: flowStats.style.color },
                              ]}
                            />
                            <ThemedText
                              semanticVariant="tableHeader"
                ...
                {totals.gasM3 !== 0 ? (
                  <View style={styles.totalsWrap}>
                    ...
                    >
                      ...
                      <NumberText size="summary" weight="semibold">
                        {
                          formatGas(totals.gasM3, unitsConfig, {
                            display: "smart",
                          }).valueText
                        }
                      </NumberText>
